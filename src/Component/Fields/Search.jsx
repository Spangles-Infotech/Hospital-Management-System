  ...
  return (
    <div className={`${!isSearch ? "hidden" :isForm ? "h-[98px]": "h-[35px] "}`}>
        <div className={`relative h-full ${isForm ? "flex justify-end items-end" : ""}`}>
            <div 
              className={`absolute flex items-center ps-2 cursor-pointer ${isBlue ? "text-[#1F9CC6] inset-y-0 start-0" : isForm ? "text-white bottom-[17%] left-[5%] " : "text-customBlack inset-y-0 start-0" }`}
            ...
            <input
                ...
                name="search"
                id="default-search"
                className={`block w-40 h-[35px] py-1 ps-10 text-sm border-2 transition-all duration-500 ease-in-out  ${isBlue ? "border-[#1F9CC6] bg-[#EAFFFE] text-[#1F9CC6] focus:outline-none rounded-md " : isForm ? "h-[50px] ps-15 rounded-full bg-[#1F9CC6] text-white placeholder:text-white border-[#1F9CC6] focus:ring-none focus:outline-none" : "ring-[#CECECE] border-[#CECECE] bg-gray-50 focus:ring-[#1F9CC6] focus:outline-[#1F9CC6] text-gray-900  rounded-md "} `}
                placeholder="Search..."
            ...
            {/* <button 
                type="button" 
                onClick={handleSubmit}
                className="text-white absolute end-0 bottom-0.5 bg-[#1F9CC6] hover:bg-[#1F9CC6]-focus focus:ring-4 focus:outline-none focus:ring-[#1F9CC6] font-medium rounded-lg text-sm px-4 py-2"
            >
                Search
            </button> */}
        </div>
